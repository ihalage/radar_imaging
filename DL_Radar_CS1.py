...
DATA_DIRECTORY='Train Libraray Experiment/data_1'		#data shape (12x12)
IMAGE_DIRECTORY='Train Libraray Experiment/image_CS_1'	#image shape (100x100)
...
class DL_Radar():
	...
	def __init__(self):
		...
		self.real_data,self.imaginary_data = [i.reshape(self.no_of_images*self.input_dimension*self.input_dimension,1) for i in (self.unzip(list(self.read_data(DATA_DIRECTORY))))]
		self.real_image,self.imaginary_image = [i.reshape(self.no_of_images*self.target_dimension*self.target_dimension,1) for i in (self.unzip(list(self.read_data(IMAGE_DIRECTORY))))]
	...
	def build_network(self, in_shape):
		...
		mag_conv1 = Conv2D(32, [3,3], strides=(1,1), padding='same', activation='relu', name='mag_conv1')(magnitude) #12x12
		mag_conv2 = Conv2D(48, [3,3], strides=(1,1), padding='same', activation='relu', name='mag_conv2')(mag_conv1) #12x12
		mag_conv3 = Conv2D(64, [3,3], strides=(1,1), padding='same', activation='relu', name='mag_conv3')(mag_conv2) #12x12

		## phase processing neural network
		phase_conv1 = Conv2D(32, [3,3], strides=(1,1), padding='same', activation='relu', name='phase_conv1')(phase) #12x12
		phase_conv2 = Conv2D(48, [3,3], strides=(1,1), padding='same', activation='relu', name='phase_conv2')(phase_conv1) #12x12
		phase_conv3 = Conv2D(64, [3,3], strides=(1,1), padding='same', activation='relu', name='phase_conv3')(phase_conv2) #12x12

		merged_layer = concatenate([mag_conv3, phase_conv3], name='merged_layer', axis=-1)

		dconv1 = Conv2DTranspose(64, [3,3], strides=(2,2), padding='same', activation='relu', name='dconv1')(merged_layer) #24x24
		dconv2 = Conv2DTranspose(32, [3,3], strides=(2,2), padding='same', activation='relu', name='dconv2')(dconv1) #48x48
		dconv3 = Conv2DTranspose(1, [3,3], strides=(2,2), padding='same', activation='relu', name='dconv3')(dconv2) #96x96

		# resize to correct shape (100x100) using Keras lambda layer and tensorflow's tf.image.resize_images
		output = Lambda(lambda image: tf.image.resize_images(image, (100, 100), 
		method = tf.image.ResizeMethod.BICUBIC,
		align_corners = True, # possibly important
		preserve_aspect_ratio = True))(dconv3)

		dl_model = Model(inputs=[magnitude, phase], outputs=[output])
	...
	def train(self, epochs, learn_rate, batch_size=20):
		...
		# all_xs = np.load('saved_arrays/CS1_all_xs.npy')
		# all_ys = np.load('saved_arrays/CS1_all_ys.npy')

		np.save('saved_arrays/CS1_all_xs', all_xs)
		np.save('saved_arrays/CS1_all_ys', all_ys)

		all_phase_xs, all_phase_ys = self.unzip(list(self.read_phase(self.input_phase_indata,self.target_phase_indata)))
		# all_phase_xs = np.load('saved_arrays/CS1_all_phase_xs.npy')
		# all_phase_ys = np.load('saved_arrays/CS1_all_phase_ys.npy')

		np.save('saved_arrays/CS1_all_phase_xs', all_phase_xs)
		np.save('saved_arrays/CS1_all_phase_ys', all_phase_ys)
...
# 
		...
		def do_report(e):
			...
			predictions = self.network.predict([self.test_xs, self.phase_test_xs])
			# plt.imshow(test_batch_ys.reshape(60,60))
			# plt.show()
			for i in range(50):
				f = plt.figure(figsize=(8,8))
				ax = f.add_subplot(2,2,1)
				ax.set_title("Received Signal Magnitude", fontsize=11)
				...
				ax.set_title("Received Signal Phase", fontsize=11)
				plt.imshow(self.phase_test_xs[i].reshape(self.input_dimension,self.input_dimension))
				plt.savefig('Scheme2_CS/epoch'+str(e)+'_test'+str(i))
				# plt.show()
				plt.close()

		checkpointer = ModelCheckpoint('saved_models/radarCS1_{epoch:0003d}.h5', period=20)

		try:
			for epoch in range(epochs):
				self.network.fit([self.train_xs, self.phase_train_xs], [self.train_ys], validation_split=0.2, epochs=100, batch_size=batch_size, callbacks=[checkpointer])
...
if __name__ == '__main__':
	dl_radar = DL_Radar()
	dl_radar.train(epochs=1000, learn_rate=0.001, batch_size=32)
